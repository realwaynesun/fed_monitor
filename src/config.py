"""
Config loader for Fed Monitor and BOJ Monitor.
...
class BojMonitorConfig:
    """Loads and provides access to boj_monitor_config.yaml."""

    def __init__(self, config_path: str | Path | None = None):
        if config_path is None:
            config_path = Path(__file__).parent.parent / "config" / "boj_monitor_config.yaml"
    ...
    @property
    def timezone(self) -> str:
        return self._raw.get("timezone", "Asia/Tokyo")

    # -------------------------------------------------------------------------
    # Data Sources
    # -------------------------------------------------------------------------
    @property
    def boj_base_url(self) -> str:
        return self._raw["data_sources"]["boj"]["base_url"]

    @property
    def boj_stat_search_url(self) -> str:
        return self._raw["data_sources"]["boj"]["stat_search_url"]

    @property
    def boj_rate_limit(self) -> dict:
        return self._raw["data_sources"]["boj"].get("rate_limit", {
    ...
    def series_by_source(self, source: str) -> list[dict]:
        """Get series filtered by data source (boj, fred, manual, etc.)."""
        return [s for s in self.series if s.get("source") == source]
    ...
    def alerts_by_category(self, category: str) -> list[dict]:
        """Filter alerts by category (rates, ycc, liquidity, fx, stress)."""
        return [a for a in self.alerts if a.get("category") == category]
    ...
    @property
    def database_path(self) -> str:
        return self._raw.get("database", {}).get("path", "boj_monitor.db")


# Singleton instance for BOJ config
_boj_config: BojMonitorConfig | None = None


def get_boj_config(config_path: str | Path | None = None) -> BojMonitorConfig:
    """Get or create the BOJ config singleton."""
    global _boj_config
    if _boj_config is None or config_path is not None:
        _boj_config = BojMonitorConfig(config_path)
    return _boj_config
...
) -> FedMonitorConfig | BojMonitorConfig:
    """
    Get config for specified monitor type.

    Args:
        monitor: "fed" for US Fed Monitor, "boj" for Japan BOJ Monitor
        config_path: Optional custom config path

    Returns:
        FedMonitorConfig or BojMonitorConfig instance
    """
    if monitor == "boj":
        return get_boj_config(config_path)
